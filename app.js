...
async function renderItems(el) {
    ...
    <div class="page-body">
      <div class="filters-bar">
        ...
        <div class="filter-chips" id="items-type-filter">
          ...
          <button class="filter-chip" data-type="set">Sæt</button>
        </div>
      </div>
      <div class="filters-bar">
  ...
  const loadItemsList = async () => {
    const thisVersion = ++loadingVersion;
    const q = document.getElementById('items-search')?.value || '';
    const grid = document.getElementById('items-grid');

    // Determine container class based on view mode
    if (grid) grid.className = viewMode === 'list' ? 'items-list' : 'items-grid';

    if (currentType === 'set') {
      // Only show sets
      const { data: sets } = await sb.from('item_sets').select('*, set_items(quantity, items(id, name, quantity, min_quantity))').order('name');
      if (thisVersion !== loadingVersion) return;
      let filteredSets = (sets || []);
      if (q) filteredSets = filteredSets.filter(s => s.name.toLowerCase().includes(q.toLowerCase()) || (s.description || '').toLowerCase().includes(q.toLowerCase()));
      renderItemsView([], filteredSets, viewMode);
      return;
    }
    ...
    const [{ data: items, error }, setsResult] = await Promise.all([
      query,
      !currentType ? sb.from('item_sets').select('*, set_items(quantity, items(id, name, quantity, min_quantity))').order('name') : Promise.resolve({ data: [] })
    ]);
    if (thisVersion !== loadingVersion) return;
    ...
    let filteredSets = (setsResult.data || []);
    if (q) filteredSets = filteredSets.filter(s => s.name.toLowerCase().includes(q.toLowerCase()) || (s.description || '').toLowerCase().includes(q.toLowerCase()));

    renderItemsView(flatItems, filteredSets, viewMode);
  };
...
function renderItemsView(items, sets, mode) {
  const grid = document.getElementById('items-grid');
  if (!grid) return;
  ...
  grid.className = mode === 'list' ? 'items-list' : 'items-grid';
  ...
  if (mode === 'list') {
      ...
      return `
        <div class="list-item" onclick="showItemDetail('${item.id}')">
          ...
          <div class="list-item-right">
            ...
            ${item.type === 'food' ? `<span class="tag food" style="font-size:0.68rem">Madvare</span>` : ''}
          </div>
        </div>`;
    }).join('');

    html += (sets || []).map(s => {
      const components = s.set_items || [];
      const hasIssues = components.some(c => c.items && c.items.quantity < c.quantity);
      return `
        <div class="list-item" onclick="showSetDetail('${s.id}')">
    ...
    html += (sets || []).map(s => {
      ...
      const hasIssues = components.some(c => c.items && c.items.quantity < c.quantity);
      return `
        <div class="item-card set-item-card" onclick="showSetDetail('${s.id}')">
          <div class="item-card-image">
  ...
  lucide.createIcons({ nodes: [grid] });
  if (isAdmin()) setupSwipeToDelete(grid);
}
...
async function showItemDetail(id) {
  try {
    ...
    openModal(`
      <div class="modal-handle"></div>
      <button class="modal-close detail-close-btn" onclick="closeModal()">${icon('x')}</button>
      ${mainImageHtml}
      ${galleryHtml}
...
async function renderTrips(el) {
  ...
  const visibilityLabels = { shared: 'Fælles', private: 'Privat' };
  const visibilityIcons = { shared: 'users', private: 'lock' };
    ...
    <div class="trip-card" onclick="showTripDetail('${t.id}')">
      <div class="trip-card-header">
        ...
        <div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px">
          ...
          <span style="font-size:0.78rem;color:var(--text-3);display:flex;align-items:center;gap:3px">${icon(visibilityIcons[t.visibility] || 'users')} ${visibilityLabels[t.visibility] || 'Fælles'}</span>
        </div>
      </div>
...
async function showTripDetail(tripId) {
  const [{ data: trip }, { data: tripItems }] = await Promise.all([
    ...
    sb.from('trip_items').select('*, items(id, name, image_url)').eq('trip_id', tripId)
  ]);
  ...
  const statusOptions = Object.keys(statusLabels).map(s =>
    `<option value="${s}" ${trip.status === s ? 'selected' : ''}>${statusLabels[s]}</option>`
  ).join('');
  ...
  openModal(`
    ...
    <div class="modal-body">
      <div class="detail-meta-grid">
        <div class="detail-meta-item">
          ...
          <div class="detail-meta-value">
            ${isAdmin() ? `<select class="form-input" style="padding:4px 8px;font-size:0.88rem" onchange="changeTripStatus('${tripId}', this.value)">${statusOptions}</select>` : `<span class="status-badge ${statusColors[trip.status]}">${statusLabels[trip.status]}</span>`}
          </div>
        </div>
        <div class="detail-meta-item">
        ...
        <div class="detail-meta-item">
          <div class="detail-meta-label">Synlighed</div>
          <div class="detail-meta-value">${icon(trip.visibility === 'private' ? 'lock' : 'users')} ${visibilityLabels[trip.visibility] || 'Fælles'}</div>
        </div>
        ${trip.description ? `<div class="detail-meta-item" style="grid-column:1/-1"><div class="detail-meta-label">Beskrivelse</div><div class="detail-meta-value">${esc(trip.description)}</div></div>` : ''}
      </div>
      ...
      ${items.map(i => {
        ...
        const name = i.items?.name || 'Ukendt';
        return `
    ...
    <div class="detail-actions">
      ${isAdmin() ? `
        ...
        <button class="btn btn-danger btn-sm" onclick="deleteTrip('${tripId}')">${icon('trash-2')} Slet tur</button>
      ` : ''}
    </div>`);
}
...
async function advanceTripStatus(tripId, newStatus) {
  await changeTripStatus(tripId, newStatus);
}
...
async function changeTripStatus(tripId, newStatus) {
  try {
    ...
    if (newStatus === 'active') {
      ...
      const statusLabels = { planning: 'Planlægning', packing: 'Pakning', active: 'Aktiv', completed: 'Afsluttet' };
      toast(`Status ændret til: ${statusLabels[newStatus] || newStatus}`);
    }
...
async function deleteTrip(tripId) {
  if (!confirm('Er du sikker på at du vil slette denne tur og hele dens pakkeliste?')) return;
  try {
    await sb.from('trip_items').delete().eq('trip_id', tripId);
    await sb.from('trips').delete().eq('id', tripId);
    await logActivity('trip_deleted', 'trip', tripId, 'Tur slettet', {});
    toast('Tur slettet');
    closeModal();
    navigate('trips');
  } catch(e) { toast(e.message, 'error'); }
...
async function showAddTripItem(tripId) {
  const [{ data: items }, { data: sets }] = await Promise.all([
    sb.from('items').select('id, name, quantity, image_url, item_categories(categories(name))').order('name'),
    sb.from('item_sets').select('id, name, image_url, description').order('name')
  ]);

  const allItems = (items || []).map(i => ({
    ...i,
    _type: 'item',
    _cats: (i.item_categories || []).map(ic => ic.categories?.name).filter(Boolean)
  }));
  const allSets = (sets || []).map(s => ({ ...s, _type: 'set' }));
  ...
  function renderPickerList(query) {
    ...
    let source = pickerTab === 'item' ? allItems : allSets;
    if (q) {
      source = source.filter(x => {
        const name = (x.name || '').toLowerCase();
        const desc = (x.description || '').toLowerCase();
        const cats = x._cats ? x._cats.join(' ').toLowerCase() : '';
        return name.includes(q) || desc.includes(q) || cats.includes(q);
      });
    }

    if (source.length === 0) {
      listEl.innerHTML = '<div class="picker-empty">' + icon('search') + '<p>Ingen resultater</p></div>';
      lucide.createIcons({ nodes: [listEl] });
      return;
    }

    listEl.innerHTML = source.map(x => {
      const isSelected = selectedId === x.id && selectedType === x._type;
      const imgHtml = x.image_url
        ? '<img src="' + (x.image_url.startsWith('http') ? x.image_url : './' + x.image_url) + '" alt="" onerror="handleImgError(this)">'
        : '<div class="placeholder-icon">' + icon(x._type === 'set' ? 'boxes' : 'package') + '</div>';
      const meta = x._type === 'item'
        ? (x.quantity != null ? x.quantity + ' stk' : '') + (x._cats.length ? ' · ' + x._cats.slice(0, 2).join(', ') : '')
        : 'Sæt';
      return '<div class="picker-row' + (isSelected ? ' selected' : '') + '" data-id="' + x.id + '" data-type="' + x._type + '">'
        + '<div class="picker-row-img">' + imgHtml + '</div>'
        + '<div class="picker-row-info">'
        + '<div class="picker-row-name">' + esc(x.name) + '</div>'
        + '<div class="picker-row-meta">' + meta + '</div>'
        + '</div>'
        + (isSelected ? '<div class="picker-check">' + icon('check-circle') + '</div>' : '')
        + '</div>';
    }).join('');
    lucide.createIcons({ nodes: [listEl] });
    ...
    listEl.querySelectorAll('.picker-row').forEach(row => {
      row.onclick = () => {
        ...
        renderPickerList(document.getElementById('trip-picker-search')?.value);
        updatePickerFooter();
      };
    });
  }
  ...
  function updatePickerFooter() {
    const footer = document.getElementById('trip-picker-footer');
    if (!footer) return;
    if (selectedId) {
      const src = selectedType === 'item' ? allItems : allSets;
      const sel = src.find(x => x.id === selectedId);
      footer.style.display = '';
      document.getElementById('trip-picker-selected-name').textContent = sel ? sel.name : '';
    } else {
  ...
  openModal(`
    ...
    <div class="modal-body" style="padding:0;display:flex;flex-direction:column;max-height:70vh">
      <div style="padding:16px 16px 0">
        <div class="picker-tabs">
          <button class="picker-tab active" data-tab="item">${icon('package')} Genstande</button>
          <button class="picker-tab" data-tab="set">${icon('boxes')} Sæt</button>
        </div>
        <div class="picker-search-wrap">
          ${icon('search')}
          <input type="text" class="picker-search" id="trip-picker-search" placeholder="Søg genstande..." autocomplete="off">
        </div>
      </div>
      <div class="picker-list" id="trip-picker-list"></div>
    </div>
    <div class="picker-footer" id="trip-picker-footer" style="display:none">
      <div class="picker-selected-info">
        <span class="picker-selected-label">Valgt:</span>
        <span class="picker-selected-name" id="trip-picker-selected-name"></span>
      </div>
      <div class="picker-fields">
        ...
        <div class="picker-field picker-field-notes">
          <label>Noter</label>
          <input type="text" class="form-input" id="trip-item-notes" placeholder="Valgfrit">
        </div>
      </div>
      <button class="btn btn-primary btn-block" id="trip-item-save">${icon('plus')} Tilføj</button>
    </div>`);

  renderPickerList('');

  document.getElementById('trip-picker-search').addEventListener('input', (e) => {
  ...
  document.querySelectorAll('.picker-tab').forEach(tab => {
    tab.onclick = () => {
      ...
      selectedId = null; selectedType = null;
      const searchEl = document.getElementById('trip-picker-search');
      searchEl.value = '';
      searchEl.placeholder = pickerTab === 'item' ? 'Søg genstande...' : 'Søg sæt...';
      renderPickerList('');
      updatePickerFooter();
    };
  });
  ...
  document.getElementById('trip-item-save').onclick = async () => {
    if (!selectedId) { toast('Vælg en genstand', 'error'); return; }
    ...
    if (selectedType === 'item') {
      ...
      try {
        const { data: components } = await sb.from('set_items').select('item_id, quantity, items(name)').eq('set_id', selectedId);
        if (!components || components.length === 0) { toast('Sættet har ingen dele', 'error'); return; }
        const { data: setInfo } = await sb.from('item_sets').select('name').eq('id', selectedId).single();
        const setName = setInfo?.name || 'Sæt';
        const rows = components.map(c => ({
          trip_id: tripId,
          item_id: c.item_id,
          quantity_needed: c.quantity * qty,
          quantity_packed: 0,
          notes: notes ? `${setName}: ${notes}` : `Del af sæt: ${setName}`
        }));
        await sb.from('trip_items').insert(rows);
        toast(`${components.length} dele fra "${setName}" tilføjet`);
        closeModal();
        setTimeout(() => showTripDetail(tripId), 100);
      } catch(e) { toast(e.message, 'error'); }
    }
...
async function showTripForm(editId) {
  ...
  openModal(`
    ...
    <div class="modal-body">
      ...
      <div class="form-group">
        ...
        <select class="form-input" id="trip-visibility">
          <option value="shared" ${(trip?.visibility || 'shared') === 'shared' ? 'selected' : ''}>Fælles – synlig for alle ledere</option>
          <option value="private" ${trip?.visibility === 'private' ? 'selected' : ''}>Privat – kun synlig for dig</option>
        </select>
      </div>
    </div>
    <div class="modal-footer">
  ...
  document.getElementById('trip-save-btn').onclick = async () => {
    ...
    const record = {
      ...
      visibility: document.getElementById('trip-visibility').value || 'shared'
    };
...
async function exportTripPDF(tripId, tripName) {
  try {
    const { data: tripItems } = await sb.from('trip_items').select('*, items(name)').eq('trip_id', tripId);
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF();
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(18);

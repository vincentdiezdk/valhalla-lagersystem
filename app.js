...
async function showAddTripItem(tripId) {
  ...
  document.getElementById('trip-item-save').onclick = async () => {
    ...
    if (type === 'item') {
      ...
      try {
        const { data: components } = await sb.from('set_items').select('item_id, quantity, items(name)').eq('set_id', setId);
        if (!components || components.length === 0) { toast('Sættet har ingen dele', 'error'); return; }
        const { data: setInfo } = await sb.from('item_sets').select('name').eq('id', setId).single();

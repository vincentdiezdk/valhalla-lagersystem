...
async function renderItems(el) {
  ...
  const loadItemsList = async () => {
    ...
    // Also load sets and show them in the grid if type filter matches
    if (!currentType || currentType === 'set') {
      const q = document.getElementById('items-search')?.value || '';
          ...
          return `
            <div class="item-card" onclick="showSetDetail('${s.id}')">
              ...
              <div class="item-card-body">
                ...
                ${hasIssues ? `<span class="expiry-badge red" style="margin-top:4px">${icon('alert-triangle')} Mangler dele</span>` : `<span class="expiry-badge green" style="margin-top:4px">${icon('check-circle')} Komplet</span>`}
              </div>
            </div>`;
        }).join('');
        if (filteredSets.length === 0) grid.innerHTML = `<div class="empty-state" style="grid-column:1/-1">${icon('boxes')}<h3>Ingen s\u00e6t</h3><p>Opret dit f\u00f8rste s\u00e6t under S\u00e6t-menuen</p></div>`;
        lucide.createIcons({ nodes: [grid] });
      } else if (filteredSets.length > 0) {
        // Append sets to the items grid
        const grid = document.getElementById('items-grid');
        const setsHtml = filteredSets.map(s => {
          const components = s.set_items || [];
          const hasIssues = components.some(c => c.items && c.items.quantity < c.quantity);
          return `
